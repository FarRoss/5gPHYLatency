#!/usr/bin/python3
#
# Usage: python plot-section4.3.py
#
...
plotme(plt, '4.3a', 'Fig7a', show_flag=SHOW_PLOT_FLAG)
plt.close(fig)
...
plotme(plt, '4.3b', 'Fig7b', show_flag=SHOW_PLOT_FLAG)
plt.close(fig)
...
plotme(plt, '4.3c', 'Fig7c', show_flag=SHOW_PLOT_FLAG)
plt.close(fig)

##########################################
# Figure 8 # Linear Relation between the UL Latency (T_UL) and code-blocks
#########################################

df = pd.read_csv('plot-section4.3-Fig8.csv')

fig = plt.figure(figsize=(5, 1.4))
gs1 = gridspec.GridSpec(1, 1, wspace=0.35, hspace=0.26, top=.97, bottom=0.19, left=0.14, right=0.99, figure=fig)
ax0 = plt.subplot(gs1[0])

x = df['T_UL Total [ms]']
y = df['Num Codeblocks']
col = df['Num Codeblocks'].astype(int)


myPlot = ax0.scatter(x, y, s=13, c=col, cmap='rainbow')
ax0.set_ylim([0, 40])
ax0.set_yticks([0, 10, 20, 30, 40])
ax0.set_xticks([0.6, 0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4])
ax0.set_xlim([0.8, 2.2])
ax0.set_xlabel('$T_{UL}$ (in ms)', fontsize=12)
ax0.set_ylabel('# of UL \nTx. Cycles', fontsize=12)
ax0.tick_params(axis='x', labelsize=12)
ax0.tick_params(axis='y', labelsize=12)
ax0.xaxis.grid(True, which='major')
ax0.yaxis.grid(True, which='major')
...
cbar.set_label('# of \nCode-blocks', rotation=270, labelpad=25, fontsize=12)
plotme(plt, '4.3', 'Fig8', show_flag=SHOW_PLOT_FLAG)
plt.close()
